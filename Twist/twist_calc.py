...
G = 26000000000 #database_connector.load_wingbox_value("shear_modulus_Pa")
...
def change_of_twist_angle(y):
    ...
    matrix = np.array([[2*Area_first*chord*chord, 2*Area_second*chord*chord, 0], 
                [(((((b_one+a_one+a_two)*chord)/t1)+(b_three*chord/t3))/2*Area_first*chord*chord*G), -1*((b_three*chord/t3)/2*Area_first*chord*chord*G), -1], 
                [-1*((b_three*chord/t3)/2*Area_second*chord*chord*G), (((((b_two+c_one+c_two)*chord/t1)+(b_three*chord/t3))/2*Area_second*chord*chord*G)), -1]])
