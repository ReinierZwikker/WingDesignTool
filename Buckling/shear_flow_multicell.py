...
def shearflow_doublecell(spanwise_location):
    ...
    inter_stringer_distance_bottom = get_length(
        [[0, 0], abs(stringer_bottom_distance_from_centroid[0] - stringer_bottom_distance_from_centroid[1])])
    inter_stringer_distance_top = get_length(
        [[0, 0], abs(stringer_top_distance_from_centroid[0] - stringer_top_distance_from_centroid[1])])
    ...
    q_tot_front_spar = abs(integral_value_front_bottom + q_t_1256 + q_s_s0_1256)

    # middle spar tot shear flow
    q_tot_middle_spar = abs(q_b(
        stringer_bottom_distance_from_centroid[int(round(len(stringer_bottom_distance_from_centroid) / 2))],
        area_top_stringer) + q_t_1256 + q_s_s0_1256 - q_t_2345 - q_s_s0_2345)

    # aft spar tot shear flow
    q_tot_aft_spar = abs(q_b(stringer_bottom_distance_from_centroid[int(round(len(stringer_top_distance_from_centroid)))
                         ],
                         area_top_stringer) + q_t_1256 + q_s_s0_1256 - q_t_2345 - q_s_s0_2345 + q_t_2345 + q_s_s0_2345)

    return q_max_top_flange_value / t_12, q_max_bottom_flange_value / t_12, max(q_tot_front_spar, q_tot_middle_spar, q_tot_aft_spar) * t_61
