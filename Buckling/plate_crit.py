...
def stringer_spacing(y):
    if y <= top_lim1:
        spacing = (sqrt((points[1][0] - points[0][0])**2 + (points[1][1] - points[0][1])**2) * chord_function(y)) / (num_1+1)
...
def plate_crit_force(y): # b is the stringer spacing
    Fcr = (((pi**2)*kc*E) / (12*(1-(pois**2)))) * ((t/stringer_spacing(y))**2)
    return Fcr


def plate_crit_stress(y): # b is the stringer spacing
