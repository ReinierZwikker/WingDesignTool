...
for spanwise_location in spanwise_locations_list:
    ...
    x_shear_force_data.append(add_engine_component(x_shear_force_function_trapz(spanwise_location, wing_span/2, global_length_step, test_density, test_velocity), spanwise_location, 1))
    ...
    print(f"Shear: {x_shear_time:.2f} s, Moment: {x_moment_time:.2f} s, Time left approx: {(x_shear_time + x_moment_time) * wing_span / (2 * global_length_step) - total_time_taken:.0f} s\n")
    total_time_taken += x_shear_time + x_moment_time
print(f"total time: {(time.time() - start_time_1)/60} min")
